...
println("testing fdtd equivalence with matrix formulation")

geo = CuboidRoom(10,20,10,[10.;20.;30.;40.;50.;60.], FDTDEnv(0.1,IWB()))
Nt = 200
s_t = randn(Nt,1)
xs = [(5, 5, 5)]
xr = [(5, 8, 7)]
f = FDTD(Float64,xr,xs,Nt,geo) 
Qm,A,Qp = get_QmAQp(geo)

p = zeros(Nt,length(xr))
p = fdtd(f,s_t)
@time p = fdtd(f,s_t)
p = fdtd(xr[1],xs[1],Nt,geo,s_t)

P = fdtd(xs,Nt,geo,s_t)
@time P = fdtd(xs,Nt,geo,s_t)
...
@test norm(p[:]-p2[:]) <1e-8
@test norm(P[:]-P2[2*geo.Nx*geo.Ny*geo.Nz+1:end]) <1e-8
